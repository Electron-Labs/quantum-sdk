...
describe("register circuit", () => {
    ...
    it("should fail when vkey path is not valid", async () => {
        ...
        return expect(quantum.registerCircuit("/doesnot/exist", "", ProofType.GNARK_GROTH16)).to.be.rejectedWith(/^VkeyPath does not exist*/);
    })

    it("should fail in serialization when vkey json doesn't correspond to vkey schema", async () => {
        const scope = nock(rpcEndPoint).post("/register_circuit");
        return expect(quantum.registerCircuit("test/dump/wrong_vkey.json", "", ProofType.GNARK_GROTH16)).to.be.rejectedWith(/^Error in serializing vkey*/);
    })
    ...
    it("should fail when node server replies with error", async () => {
        ...
        return expect(quantum.registerCircuit("test/dump/vkey.json", "", ProofType.GNARK_GROTH16)).to.be.rejectedWith(/^error in register circuit api*/);
    })
    ...
    it("should return valid circuitId when correct path to correct vkey is provided", async () => {
        ...
        const circuitIdHash = await quantum.registerCircuit("test/dump/vkey.json", "", ProofType.GNARK_GROTH16);
